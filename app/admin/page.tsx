...
import { getOverviewStats } from "@/app/(common-lib)/bigquery-queries";
    ...
    <div className="flex flex-col gap-2">
      {data.map((d, i) => (
        <div key={i} className="flex items-center gap-3">
          ...
          <span className="w-8 text-right text-xs font-bold text-white/80">
            {d.count}
          </span>
        </div>
      ))}
...
export default async function AdminOverview() {
  ...
  const noData = stats.totalLeads === 0;

  return (
    <div>
      <h1 className="text-xl font-extrabold text-white">Overview</h1>
      <p className="mt-1 text-xs text-white/40">
        Lead analytics from BigQuery · {stats.recentLeads.length} recent leads
        shown
      </p>
      ...
      {noData && (
        <div className="mt-8 rounded-2xl border border-white/10 bg-[#0d2843] p-6 text-center">
          <p className="text-sm text-white/60">
            No leads yet. Once you start receiving leads, they will appear here
            with full telemetry data.
          </p>
        </div>
      )}
        ...
        <>
          <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
            ...
            <StatCard
              ...
              value={formatTime(stats.avgTimeOnPage)}
              color="#9c27b0"
            />
          </div>
              ...
              {stats.byDay
                ...
                .map((d, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className="w-full rounded-t-sm bg-[#0d47a1] transition-all hover:bg-[#1565c0]"
                        style={{
                          height: `${Math.max((d.count / max) * 120, 2)}px`,
                        }}
                      />
                    </div>
                  );
