...
export function DiagnosticFlow({
  ...
  const problems = service
    ? (serviceProblems[service as keyof typeof serviceProblems] || [])
    : [];
      ...
      <div className="relative w-full max-w-md rounded-3xl bg-background shadow-2xl">
        ...
        <div className="flex justify-end px-4 pt-3">
          <button
            type="button"
            onClick={handleClose}
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary-container text-on-primary-container hover:bg-error hover:text-on-error transition-colors"
            aria-label="Close"
          >
        ...
        {done ? (
          ...
          <div className="flex flex-col gap-5 p-6">
            ...
            {/* What's the issue? — appears when category selected */}
            {selectedService && (
              <div>
                <p className="text-sm font-bold text-on-surface">
            ...
            {/* Name + number — appears when category selected */}
            {selectedService && (
